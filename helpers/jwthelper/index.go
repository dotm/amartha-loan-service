...
const jwtIssuer = "amartha/loan-engine-service" //for jwt registered claim: iss
const jwtAudience = "amartha-frontend"          //for jwt registered claim: aud
...
const amarthaCustomClaimName = "alcc"
const amarthaCustomClaimKeyUserId = "user_id"
...
func BuildCustomClaims(userId string) CustomClaimMap {
	return map[string]interface{}{
		amarthaCustomClaimKeyUserId: userId,
	}
}
func GetUserId(token jwt.Token) string {
	return GetCustomClaims(token)[amarthaCustomClaimKeyUserId].(string)
}
func GetCustomClaims(token jwt.Token) CustomClaimMap {
	return token.PrivateClaims()[amarthaCustomClaimName].(CustomClaimMap)
}
...
func BuildAndSign(claimValue interface{}, expiration time.Time) (string, error) {
	jwtToken, err := jwt.NewBuilder().
		...
		Expiration(expiration).Claim(amarthaCustomClaimName, claimValue).
		Build()
	if err != nil {
...
func ParseFromAuthorizationHeader(headerValue string) (jwt.Token, error) {
	if headerValue == "" {
		return nil, fmt.Errorf("user credential empty")
	}
	authSlice := strings.Split(headerValue, "Bearer ") //bearer token auth is always prefixed with "Bearer "
	if len(authSlice) != 2 {
		return nil, fmt.Errorf("user credential malformed")
	}
...
func verifySignedToken(signedJwtToken string) (jwt.Token, error) {
	...
	if err != nil {
		if err.Error() == `"exp" not satisfied` {
			return nil, fmt.Errorf("user credential expired")
		}
	...
	if jwtToken.Issuer() != jwtIssuer {
		return nil, fmt.Errorf("user credential issuer invalid")
	}
